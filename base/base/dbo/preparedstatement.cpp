...
namespace base
{
    namespace dbo
    {
        ...
        PreparedStatement* PreparedStatement::Create(int pool_id, const char* sql)
        {
            PreparedStatement* obj = new PreparedStatement(pool_id, sql);
            obj->AutoRelease();
            return obj;
        }

        PreparedStatement::PreparedStatement(int pool_id, const char* sql)
            : StatementBase(pool_id, sql), cursor_flag_(0), metadata_pstmt_(nullptr), phase_(PREPARE_NEW),
              params_(new PacketOut(g_dbpool->mempool(), 200)), params_idx_(0)
        {
            params_->SkipTo(0);
        }
        ...
        void PreparedStatement::SetDateTime(const tm& dt)
        {
            if (dt.tm_hour != 0 || dt.tm_min != 0 || dt.tm_sec != 0) {
                ...
                params_->WriteFixedInteger<2>(dt.tm_year + 1900);
                params_->WriteFixedInteger<1>(dt.tm_mon + 1);
            ...
            switch (phase_) {
                ...
                case PREPARE_WAIT_EXEC_RESPONSE: {
                    if (rs_.Parse(pktin, dbo::internal::PROTOCOL_BINARY)) {
                        if (rs_.warning_count() > 0) {
                            LOG_ERROR("SQL=[%s] warnings count:%u, %s\n", sql_.c_str(), rs_.warning_count(), rs_.warning_info());
                        }
                        HandleFinish();
                    }
                }
                break;
                default: {
                    LOG_ERROR("dbo: unexception packet receive, may be too busy.");
                }
                break;
            }
        }
        ...
        void PreparedStatement::HandlePrepareFinish()
        {
            ...
            if (metadata_pstmt_->num_params != params_idx_) {
                LOG_ERROR("dbo: prepared statement params number missmatch, expect=%u, got=%u!", metadata_pstmt_->num_columns, params_idx_);
            }
            ResetSequenceID();
