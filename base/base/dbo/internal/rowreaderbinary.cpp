...
namespace base
{
    namespace dbo
    {
        namespace internal
        {
            ...
            void RowReaderBinary::Dump()
            {
                if (fields_ == nullptr) {
                    return;
                }
                for (uint32_t i = 0; i < metadata_.num_columns; ++i) {
                    if (IsNull(i)) {
                        cout << i << "[NULL]";
                        continue;
                    }
                    FieldValue* field = fields_ + i;
                    const ColumnDefinition& cm = metadata_.GetColumnDefinition(i);
                    switch (cm.type) {
                        case MYSQL_TYPE_TINY:
                            cout << i << "[int8]" << (int32_t)field->int8;
                            break;
                        case MYSQL_TYPE_SHORT:
                            cout << i << ".[int16]" << (int32_t)field->int16;
                            break;
                        case MYSQL_TYPE_LONG:
                            cout << i << ".[int32]" << (int32_t)field->int32;
                            break;
                        case MYSQL_TYPE_LONGLONG:
                            cout << i << ".[int64]" << (int64_t)field->int64;
                            break;
                        case MYSQL_TYPE_FLOAT:
                            cout << i << ".[float]" << field->v_float;
                            break;
                        case MYSQL_TYPE_DOUBLE:
                            cout << i << ".[double]" << field->v_double;
                            break;
                        case MYSQL_TYPE_INT24:
                            cout << i << ".[INT24]" << field->int32;
                            break;
                        case MYSQL_TYPE_YEAR:
                            cout << i << ".[YEAR]" << field->int16;
                            break;
                        ...
                        case MYSQL_TYPE_STRING: {
                            string ret;
                            pktin_.SkipTo(field->buffer_pos);
                ...
                for (uint32_t column = 0; column < metadata_.num_columns; ++column) {
                    ...
                    if (!IsNull(column)) {
                        switch (cm.type) {
                            ...
                            default: {
                                string msg;
                                base::utils::string_append_format(msg, "not support type = %d", (int)cm.type);
                                throw Exception(msg);
                            }
                            break;
                        }
