...
namespace base
{
        ...
        : level_(LogLevelDebug), dir_("logs"), fd_(NULL)
    {
		...
		{
			filename_ = strrchr(strProcessPath, '/') + 1;
		}
    }
    ...
    bool FileLogger::OpenLogFile()
    {
		...
		sprintf(filename + strlen(filename), "/log_%s_%d.%.2d.%.2d",
			filename_.empty() ? "" : filename_.c_str(), start_.tm_year + 1900, start_.tm_mon + 1, start_.tm_mday);
