...
namespace base
{
    namespace utils
    {
        ...
        // 压缩, level 1-9 压缩级别
        std::string zlib_compress(const char* data, size_t len, int level = 5);
        inline std::string zlib_compress(const std::string& data, int level = 5)
        {
            return zlib_compress(data.c_str(), data.size(), level);
        }

        // 解压缩
        std::string zlib_uncompress(const char* data, size_t len);
        inline std::string zlib_uncompress(const std::string& data)
        {
            return zlib_uncompress(data.c_str(), data.size());
        }
