...
#ifndef CLIENT_PACKET_BASE_H
#define CLIENT_PACKET_BASE_H
...
namespace client
{
        typedef std::vector<RefMemoryChunk> packet_data_t;
        ...
        public:
            ...
            const std::vector<RefMemoryChunk>& data() const {
                return data_;
            }
        ...
        public:
            ...
            PacketOutBase(uint32_t approx_size, MemoryPool& mempool)
                : mempool_(mempool) {
                AquireMemory(approx_size);
            }
            PacketOutBase(packet_data_t& data, MemoryPool& mempool): mempool_(mempool) {
                data_ = data;
                for (RefMemoryChunk & ck : data_) {
                    size_ += ck.count();
                }
                pos_y_ = data.size();
                pos_ = size_;
        ...
        template<typename A>
        inline void MultiWrite(client::PacketOutBase& pktout, const A& a)
        {
            pktout << a;
        }

        template<typename A, typename...Args>
        inline void MultiWrite(client::PacketOutBase& pktout, const A& a, const Args& ... args)
        {
            pktout << a;
        ...
        template<typename A>
        inline void MultiRead(client::PacketInBase& pktin, A& a)
        {
            pktin >> a;
        }

        template<typename A, typename...Args>
        inline void MultiRead(client::PacketInBase& pktin, A& a, Args& ...args)
        {
            pktin >> a;
...
#define PACKET_DEFINE_OUT(...) \
    public: \
    virtual void WriteToPacketOut(client::PacketOutBase& pktout) const {\
        client::MultiWrite(pktout, __VA_ARGS__);\
    }

#define PACKET_DEFINE_IN(...) \
    public: \
    virtual void ReadFromPacketIn(client::PacketInBase& pktin) {\
        client::MultiRead(pktin, __VA_ARGS__);\
    }
...
#define PACKET_DEFINE_CHILD_OUT(parent, ...) \
    public: \
    virtual void WriteToPacketOut(client::PacketOutBase& pktout) const override {\
...
#define PACKET_DEFINE_CHILD_IN(parent, ...) \
    public: \
    virtual void ReadFromPacketIn(client::PacketInBase& pktin) override {\
