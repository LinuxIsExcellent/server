...
namespace client
{
    ...
    {
    public:
        PacketIn(std::vector<RefMemoryChunk>& data)
        ...
        void ReadHead() {
            SkipTo(4);
            code_ = ReadUInt16();
            int16_t signature = ReadInt16();
            sessionid_ = ReadUInt16();
            if (signature != CalculateSignature()) {
                throw std::invalid_argument("bad packet signature, the packet is broken");
            }
        }
    ...
    public:
        ...
        // mempool 内存池
        PacketOut(uint16_t code, uint32_t approx_size, MemoryPool& mempool)
        ...
        static const uint16_t IDENTIFIER = 56430;   // 110, 220, ...
        ...
        void WriteHead(bool rewrite = false) {
            if (!head_writed_ || rewrite) {
                ...
                WriteUInt16(size());
                WriteUInt16(code());
                WriteInt16(CalculateSignature());
                WriteUInt16(sessionid_);
                head_writed_ = true;
            }
        }
